...
func TestFilter_Classify_UnknownSender(t *testing.T) {
	...
	c, err := f.Classify(ctx, acct, "email-1", email.Message{
	...
	s, err := senderRepo.Get(ctx, acct, "stranger@example.com")
	require.NoError(t, err)
	require.NotNil(t, s)
...
func TestFilter_Classify_IncrementsSeenCountAcrossCalls(t *testing.T) {
	...
	_, err := f.Classify(ctx, acct, "e1", msg)
	require.NoError(t, err)
	_, err = f.Classify(ctx, acct, "e2", msg)
	require.NoError(t, err)

	s, err := senderRepo.Get(ctx, acct, "repeat@example.com")
	require.NoError(t, err)
	require.NotNil(t, s)
...
func TestFilter_Classify_AppliesSenderBonus(t *testing.T) {
	...
	require.NoError(t, senderRepo.Upsert(ctx, domain.Sender{
		ID: "s-1", AccountID: acct, Email: "boss@example.com", ImportanceScore: 50, SeenCount: 5, UpdatedAt: time.Now(),
	}))

	c, err := f.Classify(ctx, acct, "e1", email.Message{
	...
	s, err := senderRepo.Get(ctx, acct, "boss@example.com")
	require.NoError(t, err)
