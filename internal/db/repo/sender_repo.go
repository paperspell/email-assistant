...
// Get retrieves the sender record for emailAddr.
// Returns nil, nil if not found.
func (r *SenderRepo) Get(ctx context.Context, emailAddr string) (*domain.Sender, error) {
	const q = `
		SELECT id, email, importance_score, seen_count, updated_at
		FROM senders WHERE email = ?
	`
	row := r.db.QueryRowContext(ctx, q, emailAddr)

	var s domain.Sender
	var updatedAtStr string
	err := row.Scan(&s.ID, &s.Email, &s.ImportanceScore, &s.SeenCount, &updatedAtStr)
...
// Upsert inserts or updates a sender record.
func (r *SenderRepo) Upsert(ctx context.Context, s domain.Sender) error {
	...
	_, err := r.db.ExecContext(ctx, q,
		s.ID, s.Email, s.ImportanceScore, s.SeenCount,
