...
func runInit(ctx context.Context, path string) error {
	if _, err := os.Stat(path); err == nil {
		return fmt.Errorf("database already exists at %s\nuse 'email-agent config set' to update settings", path)
	}
	...
	if !saved {
		fmt.Printf("Keychain is not available. Store this key and set it as %s before running the daemon:\n", keychain.EnvKey)
	...
	botToken, err := promptPassword("  Bot token")
	if err != nil {
